...
const occurrences = require('./data/occurrenceData.json');
...
const resolvableIDs = ['org.cool', 'grbio.org', 'gbif.org', 'biocol'];
let withUrlId = occurrences.filter(x => {
  if (x.institutionid && x.institutionid.match(/gbif|grbio|biocol|cool/i)) return true;
  if (x.institutioncode && x.institutioncode.match(/gbif|grbio|biocol|cool/i)) return true;
});
console.log('with likely resolvable id: ', _.sum(withUrlId.map(x => Number(x.occurrences))));